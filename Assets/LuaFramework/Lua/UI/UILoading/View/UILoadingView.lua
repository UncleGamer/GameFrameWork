...
local function OnCreate(self)
	base.OnCreate(self)
	-- 初始化各个组件
	self.loading_text = UIUtil.FindText(self.gameObject, loading_text_path)
	self.loading_slider = UIUtil.FindSlider(self.gameObject, loading_slider_path)
	...
	self.timer_action = function(self)
		self.loading_text.text = circulator()
		print('self.loading_text.text = ', self.loading_text.text)
	end
	self.timer = TimerManager:GetInstance():GetTimer(0.5, self.timer_action , self)
	self.timer:Start()
end
...
local function OnUpdate(self, delta_time)
	self.loading_slider.value = self.model.value
end
...
UILoadingView.OnUpdate = OnUpdate
