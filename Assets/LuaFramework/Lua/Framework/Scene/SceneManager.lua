...
local function __init(self)
	...
	self.scenes = {}
	--c# SceneManager
	self.cs_scene_manager = nil
end

-- 切换场景：调用c#的接口
local function __switchScene(self, scene_config)
	-- 打开loading界面
	...
	--拿到cs的场景管理器
	if not self.cs_scene_manager then
		self.cs_scene_manager = LuaHelper.GetSceneManager()
		self.cs_scene_manager.startLoadActon = function ()
			logic_scene:OnEnter()
		end
		self.cs_scene_manager.endLoadActon = function ()
			logic_scene:OnComplete()
			-- 加载完成，关闭loading界面
			uimgr_instance:DestroyWindow(UIWindowNames.UILoading)
		end
		self.cs_scene_manager.progressLoadActon = function (progress)
			model.value = progress
		end
	end
	assert(self.cs_scene_manager ~= nil)

	--切换场景
	self.cs_scene_manager:AsyncSwitchScene(scene_config.Name)
...
local function SwitchScene(self, scene_config)
	...
	self.busing = true
	__switchScene(self, scene_config)
