...
-- 分辨率
local Resolution = Vector2.New(720, 1280)
...
local function __init(self)
	...
	-- 注册upadte
	local updateFunction = function (delta_time)
		UIManager:GetInstance():OnUpdate(delta_time)
	end
	LuaHelper.GetPanelManager().updateFunction = updateFunction
end
...
local function OnUpdate(self, delta_time)
	for _,v in pairs(self.open_windows) do
		v:OnUpdate(delta_time)
	end
end
...
local function ActivateWindow(self, target, ...)
	...
	self.open_windows[target.Name] = target.View
...
local function Deactivate(self, target)
	...
	self.open_windows[target.Name] = nil
...
local function InnerOpenWindow(self, target, ...)
	...
	elseif not target.IsLoading then
		...
		LuaHelper.GetPanelManager():LoadUIPanel(target.PrefabPath, function(go)
			if IsNull(go) then
				return
			end
