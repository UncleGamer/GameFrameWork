...
namespace LuaFramework {
	public class ResourceManager : Manager {
        ...
        //资源路径对bundle的映射
        private Dictionary<string, string> assetsToBundleDics;

        public Dictionary<string, Dictionary<string, List<string>>> debugFilePath = null;
        void Awake() {
		}
		...
		public void Initialize() {
            ...
            //2.解析资源对应的builds关系
            string resPath = Application.dataPath + "/" + AppConst.AssetDir + "/assets.txt";
            if (File.Exists(resPath))
            {
                string assets = File.ReadAllText(resPath);
                assetsToBundleDics = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, string>>(assets);
            }
        ...
        public void LoadGameObject(string assetNames, LuaFunction func)
        {
            if (func != null)
                func.Call(LoadAsset<UObject>(assetNames));
        }
        ...
        /// <param name="assetNames"></param>
        /// <returns></returns>
        public GameObject LoadGameObject(string assetNames)
        {
            return LoadAsset<GameObject>(assetNames);
        }
        /// <summary>
        /// 加载声音
        /// </summary>
        /// <param name="assetNames"></param>
        /// <returns></returns>
        public AudioClip LoadAudioClip(string assetNames)
        {
            return LoadAsset<AudioClip>(assetNames);
        }
        /// <summary>
        /// 载入素材
        /// </summary>
        ...
        {
            assetname = ("Assets/" + AppConst.ResTempDir + assetname);
            Util.Log($"LoadAsset -> {assetname}");
#if UNITY_EDITOR
            ...
            assetname = assetname.ToLower();
            string abname = "";
            if(assetsToBundleDics.TryGetValue(assetname, out abname))
            {
                AssetBundle bundle = LoadAssetBundle(abname);
                return bundle.LoadAsset<T>(assetname);
            }
            ...
            {
                Util.LogError($"LoadAsset -> not found asset:{assetname}");
            }
            return null;
        }

// 
//         private T LoadFromEditor<T>(string abname, string assetname) where T : UnityEngine.Object
//         {
// #if UNITY_EDITOR
//             if (!abname.EndsWith(AppConst.ExtName))
//             {
...
//             {
//                 debugFilePath = new Dictionary<string, Dictionary<string, List<string>>>();
//                 string[] files = File.ReadAllLines(AppConst.deBugFilePath + ".txt");
//                 for (int i = 0; i < files.Length; i++)
//                 {
//                     string[] tempPath = files[i].Split('|');
//                     string keyText = tempPath[0].ToLower();
// 
//                     if (!debugFilePath.ContainsKey(keyText))
//                     {
//                         debugFilePath[keyText] = new Dictionary<string, List<string>>();
//                     }
// 
//                     string k1 = tempPath[1];
//                     if (!debugFilePath[keyText].ContainsKey(k1))
//                     {
//                         debugFilePath[keyText][k1] = new List<string>();
//                     }
//                     debugFilePath[keyText][k1].Add(tempPath[2]);
//                 }
//             }
// 
//             if (debugFilePath.ContainsKey(abname))
//             {
//                 if (debugFilePath[abname].ContainsKey(assetname))
//                 {
//                     List<string> fileList = debugFilePath[abname][assetname];
//                     for (int i = 0; i < fileList.Count; i++)
//                     {
//                         bool isLoad = true;
//                         string assetPath = fileList[i];
//                         if (!assetPath.EndsWith(".prefab"))
//                         {
//                             string tempEndStr = Path.GetExtension(assetPath);
//                             if (abname != "ui" + AppConst.ExtName && abname != "card" + AppConst.ExtName)
//                             {
//                                 if (abname != "audio" + AppConst.ExtName)
//                                 {
//                                     if (!abname.EndsWith(tempEndStr + AppConst.ExtName)
//                                             && tempEndStr != ".mp3" && tempEndStr != ".ogg" && tempEndStr != ".png")
//                                     {
//                                         isLoad = false;
//                                     }
//                                 }
...
//                             else
//                             {
//                                 Sprite obj = UnityEditor.AssetDatabase.LoadAssetAtPath<Sprite>(assetPath);
//                                 return obj as T;
//                             }
//                         }
//                         if (isLoad)
